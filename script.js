...
// ===== MAP SELECTION (host picks a map before hosting) =====
// Map 1 is whatever is already in the #trackcode div (the default map shipped
// with the page). Map 2 and Map 3 are alternate built-in tracks. "Custom map"
// lets the host paste their own track code (same format as the editor exports).
var DEFAULT_MAP_DATA = (function(){
...
chooseMapThenHost = function(){
	...
	setTimeout(function(){
		f.innerHTML = "<div class='info title'>Choose a map</div>" +
			"<div id='mapscreen'>" +
			"<div id='mapcards'>" +
			"<div class='mapcard' onclick='selectMap(1)'><div class='mappreview'>" + MAP1_PREVIEW_SVG + "</div><div class='mapcard-label'>Map 1<br>(default)</div></div>" +
			"<div class='mapcard' onclick='selectMap(2)'><div class='mappreview'>" + MAP2_PREVIEW_SVG + "</div><div class='mapcard-label'>Map 2</div></div>" +
			"<div class='mapcard' onclick='selectMap(3)'><div class='mappreview'>" + MAP3_PREVIEW_SVG + "</div><div class='mapcard-label'>Map 3</div></div>" +
			"</div>" +
			"<div id='custommapbar'>" +
...
selectMap = function(which){
	...
	if(which == 1){
		...
		trackEl.innerHTML = MAP2_DATA;
	}else if(which == 3){
		trackEl.innerHTML = MAP3_DATA;
	}else if(which == 0){
		var custom = document.getElementById("custommapinput").value.trim();
		if(custom == ""){
			alert("Paste a custom map code first.");
			return;
		}
		trackEl.innerHTML = custom;
	}
	host();
}
...
// ===== MAP RECORD KEY =====
// Simple 32-bit string hash (djb2) so each distinct track's code maps to a short,
// Firebase-safe key. Used so records are scoped per-track instead of colliding.
function hashString(str){
	...
	return (hash >>> 0).toString(36); // unsigned, base36 keeps it short + alnum only
}

// Builds the Firebase key used to store/fetch best-lap records.
// Combines a hash of the actual track code (so every custom map gets its own
// record) with the active speed multiplier (so different speed settings don't
// share a record either). Firebase keys can't contain '.', so the multiplier
// is stored as an integer (e.g. 1.25x -> "125").
...
menu2 = function(){
	...
	f.style.transform = "translate3d(0, -100vh, 0)";
	setTimeout(function(){
		f.innerHTML = "<div class='menuitem title button' id='host' ontouchstart='this.click()' onclick='chooseMapThenHost()'>Host a game</div><div class='menuitem title button' ontouchstart='this.click()' id='join' onclick='joinGame()'>Join a game</div>";
		f.style.transform = "none";
		setTimeout(function(){
					...
					if(play.data.lap > LAPS && document.getElementById("countdown").innerHTML == ""){
						...
						if(play == players[me.ref.path.pieces_[2]] && !window._myFinishTime){
							...
							if(!window._overallBestLap || window._myFinishTime < window._overallBestLap){
								...
								var ltOverallEl = document.getElementById("lt-overall");
								if(ltOverallEl) ltOverallEl.textContent = "RECORD  " + fmtLapTime(window._myFinishTime) + "  (" + me.data.name + ")";
							}
